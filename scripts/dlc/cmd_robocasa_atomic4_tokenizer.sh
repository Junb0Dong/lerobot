#!/usr/bin/env bash
# Tokenizer-only four-task robocasa atomic job. Convert/merge if needed, then train.
# Usage:
#   bash scripts/dlc/submit.sh scripts/dlc/cmd_robocasa_atomic4_tokenizer.sh
...
DISPLAY_NAME="lerobot-ac-atomic4-tok-${STAGE}-$(date '+%Y%m%d-%H%M%S')"
