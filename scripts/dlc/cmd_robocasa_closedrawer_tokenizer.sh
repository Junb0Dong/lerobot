#!/usr/bin/env bash
# Tokenizer-only CloseDrawer job. Convert v2.1→v3 if needed, then train.
# Usage:
#   bash scripts/dlc/submit.sh scripts/dlc/cmd_robocasa_closedrawer_tokenizer.sh
#   STAGE=full bash scripts/dlc/submit.sh scripts/dlc/cmd_robocasa_closedrawer_tokenizer.sh

# shellcheck disable=SC1091
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/paths.sh"

DISPLAY_NAME="lerobot-ac-${ROBOCASA_TASK,,}-tok-${STAGE}-$(date '+%Y%m%d-%H%M%S')"
