...
OUTPUT_DIR="${OUTPUT_DIR:-$(pwd)/outputs/my_dataset_merged_0902_no_head_96x128/tokenizer_fsq_overlap_phys_token16_full20k}"
...
CUDA_VISIBLE_DEVICES="${GPU}" uv run --no-sync lerobot-train-actioncodec-tokenizer \
  --repo_id="${REPO_ID}" \
  --root="${DATASET_ROOT}" \
  --output_dir="${OUTPUT_DIR}" \
  --action_dim=12 \
  --action_horizon=20 --latent_horizon=16 \
  --quantizer_type=semantic_fsq --fsq_levels='[8,5,5,5]' \
  --model_dim=512 --codebook_size=1000 --num_codebooks=1 \
  ...
  --device=cuda --amp=true --amp_dtype=bfloat16 --seed=42 --log_freq="${LOG_FREQ}"
