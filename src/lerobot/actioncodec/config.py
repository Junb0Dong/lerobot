...
class ActionCodecTokenizerConfig:
    ...
    Attributes:
        ...
        steps: Number of optimizer steps the trainer runs.
        batch_size: Number of action windows per training batch.
...
def load_tokenizer_config(directory: str | Path) -> ActionCodecTokenizerConfig:
    ...
    config = ActionCodecTokenizerConfig(**payload)
    config.validate()
    return config
